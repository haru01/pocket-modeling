...
あるエンジニアが、勉強会を継続開催したいと考え、プラットフォーム上に **コミュニティ「東京 DDD 勉強会」** を作成する。参加希望者は自由にコミュニティへ参加し、メンバーとなる。
...
CONTEXT community
  LANGUAGE    Community  = "コミュニティの永続的なグループ（メンバー・主催者・公開設定）"
  LANGUAGE    Membership = "メンバーがコミュニティに属している状態"
  MODULE      community
  UPSTREAM    (none)
  DOWNSTREAM  (none)  # Ticket がメンバー状態をスナップショット保持するため伝播不要

CONTEXT event-planning
  LANGUAGE    Event    = "単発の開催単位（タイトル・日時・会場・定員・料金）"
  LANGUAGE    Capacity = "会場枠とオンライン枠の独立管理単位"
  MODULE      event-planning
  UPSTREAM    community         # Customer-Supplier
  DOWNSTREAM  registration      # Customer-Supplier
  DOWNSTREAM  ticketing         # Customer-Supplier
  DOWNSTREAM  attendance        # Customer-Supplier

CONTEXT registration
  LANGUAGE    Application = "参加申込の状態を保持するエンティティ"
  LANGUAGE    Waitlist    = "participationType 単位のキャンセル待ち順序付きキュー"
  MODULE      registration
  UPSTREAM    event-planning    # Customer-Supplier
  DOWNSTREAM  ticketing         # Customer-Supplier

CONTEXT ticketing
  LANGUAGE    Ticket  = "確定参加権の証憑（priceSnapshot, memberAtPurchase 凍結）"
  LANGUAGE    Payment = "外部決済の試行記録"
  LANGUAGE    Refund  = "返金の試行記録"
  MODULE      ticketing
  UPSTREAM    event-planning    # Customer-Supplier（料金参照）
  UPSTREAM    registration      # Customer-Supplier（ApplicationSubmitted 受信）
  DOWNSTREAM  registration      # Customer-Supplier（確定/キャンセル完了を逆方向通知）
  DOWNSTREAM  attendance        # Customer-Supplier（Ticket 参照）

CONTEXT attendance
  LANGUAGE    Attendance = "当日の参加確認記録（会場 or オンライン）"
  LANGUAGE    CheckIn    = "会場チェックイン"
  LANGUAGE    OnlineJoin = "オンライン参加記録"
  MODULE      attendance
  UPSTREAM    ticketing         # Customer-Supplier
  UPSTREAM    event-planning    # Customer-Supplier
  DOWNSTREAM  (none)
...
SCENARIO 参加者がコミュニティに参加する
  ACTOR Member
  CMD   JoinCommunity
  EVT   MemberJoined
  AGG   Community
  # 公開済みコミュニティに対してのみ参加可能
  RULE  community must be in PUBLISHED status
  # 既存メンバーの二重参加を禁止
  RULE  member must not already be a member
  ERR   notPublished → CommunityNotAvailableError
  ERR   duplicateMember → AlreadyMemberError

SCENARIO メンバーがコミュニティを退会する
  ACTOR Member
  CMD   LeaveCommunity
  EVT   MemberLeft
  AGG   Community
  # 主催者は退会できない（先に ARCHIVE）
  RULE  member must be an active member and not the owner
  ERR   notMember → NotMemberError
  ERR   isOwner → OwnerCannotLeaveError
  # 既存 Ticket は memberAtPurchase で凍結保持されるため、退会の伝播は不要（POLICY なし）
...
SCENARIO 主催者がイベントを作成する
  ...
  ERR   pastDateTime → InvalidScheduleError
...
SCENARIO 主催者がイベントを公開する
  ...
  # 必須項目が揃っていること（onlineCapacity > 0 なら streamUrl 必須など）
  RULE  all required fields must be filled
  ERR   alreadyPublished → InvalidStatusTransitionError
  ERR   incompleteData → InvalidEventDataError
...
SCENARIO 主催者が定員を増やす
  ...
  ERR   notIncrease → InvalidCapacityError
  POL   PromoteWaitlistOnCapacityIncrease

SCENARIO 主催者が日時を変更する
  ACTOR Organizer
  CMD   RescheduleEvent
  ...
  ERR   invalidStatus → InvalidStatusTransitionError
  ERR   pastDateTime → InvalidScheduleError
  POL   ReconfirmOnEventReschedule

SCENARIO 主催者が会場を変更する
  ACTOR Organizer
  CMD   RelocateEvent
  EVT   EventRelocated
  AGG   Event
  # 開催前のみ変更可
  RULE  event must be PUBLISHED and not started
  # participationType 構成（会場枠/オンライン枠）の変更は不可
  RULE  must not change participation type composition
  ERR   invalidStatus → InvalidStatusTransitionError
  ERR   typeChange → ParticipationTypeChangeNotAllowedError
  POL   NotifyOnEventRelocation

SCENARIO 主催者が料金を変更する
  ACTOR Organizer
  CMD   RepriceEvent
  EVT   EventRepriced
  AGG   Event
  # 開催前のみ変更可
  RULE  event must be PUBLISHED and not started
  # 既存申込の Ticket は priceSnapshot で凍結、変更は新規申込にのみ適用
  RULE  price change applies only to future applications
  ERR   invalidStatus → InvalidStatusTransitionError
  POL   ReconfirmOnEventReprice

SCENARIO 主催者がイベント中止を要求する
  ACTOR Organizer
  ...
  ERR   alreadyCancelled → EventAlreadyCancelledError
  POL   RefundAllOnEventCancellation

SCENARIO システムが全返金完了でイベント中止を確定する
  ACTOR System
  CMD   CompleteEventCancellation
  EVT   EventCancellationCompleted
  AGG   Event
  # 全 confirmed 申込の返金が完了している
  RULE  all confirmed applications must have been refunded
  ERR   pendingRefunds → RefundsPendingError

SCENARIO 主催者がイベントを開催完了にする
  ACTOR Organizer
  CMD   CompleteEvent
  EVT   EventCompleted
  AGG   Event
  ...
  ERR   notYetStarted → EventNotStartedYetError
  ERR   invalidStatus → InvalidStatusTransitionError

# 定員増枠でキャンセル待ちを BULK 繰り上げ
POLICY PromoteWaitlistOnCapacityIncrease
...
# 日時変更で confirmed 申込へ再同意を BULK 要求
POLICY ReconfirmOnEventReschedule
...
# 会場変更は通知のみ（再同意不要）
# 副作用専用 POLICY: 外部通知サービス（メール / プッシュ）を recipient ごとに呼び出す。
# AGG 更新は伴わないため CMD / SCENARIO は宣言せず、ChangeNotified を観測用に発行するのみ。
...
# 料金変更で confirmed 申込へ再同意を BULK 要求
POLICY ReconfirmOnEventReprice
...
# イベント中止で全 confirmed 申込を BULK 返金（reason = EVENT_CANCEL）
POLICY RefundAllOnEventCancellation
  TRIGGER  EventCancellationRequested
  QRY      GetConfirmedApplications
  CMD      ExecuteRefund
  BULK     true

# 全 confirmed 申込の返金完了でイベント中止を確定
# [?] Saga インスタンス管理: eventId 単位で EventRefundCompleted を累積し、
#     QRY で残返金件数を確認 → ゼロ件で CompleteEventCancellation を発行
POLICY CompleteEventCancellationOnAllRefunded
...
SCENARIO 参加者がイベント詳細を確認して参加を申し込む
  ACTOR Member
  QRY   GetEventDetails
  ...
  # 同一イベントへの active 申込は1つまで
  RULE  member must not have existing active application for same event
  # 選択した participationType の枠が存在する
  RULE  participationType must match an available capacity type
  ERR   eventNotPublished → EventNotAvailableError
  ERR   duplicateApplication → AlreadyAppliedError
  ERR   noSuchCapacityType → InvalidParticipationTypeError
  # 枠あり → APPLIED、枠なし → WAITLISTED（同一トランザクション）
  WHEN  remainingCapacity > 0 → EVT ApplicationSubmitted   → POL ProcessPaymentForApplication
  WHEN  remainingCapacity = 0 → EVT ApplicationWaitlisted

SCENARIO 参加者がキャンセルを要求する
  ACTOR Member
  CMD   RequestApplicationCancellation
  EVT   ApplicationCancellationRequested
  AGG   Application
  # CONFIRMED または PENDING_RECONFIRMATION のみキャンセル要求可
  RULE  application must be in CONFIRMED or PENDING_RECONFIRMATION status
  # 開催開始後は不可
  RULE  event must not have already started
  ERR   invalidStatus → InvalidStatusTransitionError
  ERR   eventAlreadyStarted → EventAlreadyStartedError
  POL   RefundOnApplicationCancellation

SCENARIO 参加者がキャンセル待ちを取下げる
  ACTOR Member
  CMD   WithdrawFromWaitlist
  EVT   WaitlistEntryRemoved
  AGG   Application
  # WAITLISTED 状態のみ取下げ可（決済前なので返金不要）
  RULE  application must be in WAITLISTED status
  ERR   invalidStatus → InvalidStatusTransitionError

SCENARIO システムが返金完了をうけてキャンセルを確定する
  ACTOR System
  CMD   CompleteApplicationCancellation
  EVT   ApplicationCancellationCompleted
  AGG   Application
  # 個別キャンセル経由の返金完了でのみ発火（イベント中止経由は別 POLICY が処理）
  RULE  refund must have been completed with reason = MEMBER_CANCEL
  POL   PromoteWaitlistOnCancellation

SCENARIO システムがキャンセル待ちを繰り上げる
  ACTOR System
  CMD   PromoteWaitlistEntry
  EVT   WaitlistPromoted
  AGG   Application
  # WAITLISTED 状態のみ
  RULE  application must be in WAITLISTED status
  # 繰り上げ後は PROMOTED 状態に遷移し、24h の決済期限を設定
  RULE  paymentDeadline is set to 24 hours after promotion
  ERR   invalidStatus → InvalidStatusTransitionError
  POL   ProcessPaymentForPromotion

SCENARIO 参加者が変更通知を受けて継続参加を選択する
  ACTOR Member
...
SCENARIO システムが再同意を要求する
  ACTOR System
  CMD   RequestReconfirmation
  EVT   ReconfirmationRequested
  AGG   Application
  # 対象は CONFIRMED 状態の Application
  RULE  application must be in CONFIRMED status
  # CONFIRMED → PENDING_RECONFIRMATION へ状態遷移
  RULE  status transitions from CONFIRMED to PENDING_RECONFIRMATION
  ERR   invalidStatus → InvalidStatusTransitionError

SCENARIO システムがイベント中止に伴い申込をキャンセルする
  ACTOR System
  ...
  # 対象は CONFIRMED 状態の Application
  RULE  application must be in CONFIRMED status
  # status を CANCELLED に遷移（繰り上げ POLICY は発火しない別 EVT）
  ERR   invalidStatus → InvalidStatusTransitionError

# キャンセル要求を受けて返金処理を起動（CMD ExecuteRefund は reason = MEMBER_CANCEL で発行）
POLICY RefundOnApplicationCancellation
  TRIGGER  ApplicationCancellationRequested
  CMD      ExecuteRefund

# 個別キャンセル経由のキャンセル確定で空席発生 → 1 名繰り上げ
POLICY PromoteWaitlistOnCancellation
  TRIGGER  ApplicationCancellationCompleted
  QRY      GetNextWaitlistedApplication
  CMD      PromoteWaitlistEntry

# 決済期限切れによる失効でも空席発生 → 1 名繰り上げ
POLICY PromoteWaitlistOnExpiration
  TRIGGER  ApplicationExpired
  QRY      GetNextWaitlistedApplication
  CMD      PromoteWaitlistEntry

# イベント中止確定で関連 confirmed 申込を一括 CANCELLED に（BULK・繰り上げ抑止）
POLICY CancelApplicationsOnEventCancellation
...
SCENARIO システムが決済を実行する
  ACTOR System
  CMD   ExecutePayment
  AGG   Payment
  # 金額は Ticket の priceSnapshot（または Event 料金）と一致
  RULE  payment amount must match expected price
  # Application が参照する Payment は最大1つの COMPLETED まで
  RULE  must not have existing COMPLETED Payment for same Application
  ERR   amountMismatch → PaymentAmountMismatchError
  ERR   alreadyCompleted → PaymentAlreadyCompletedError
  # カード成功 → PaymentCompleted、カード失敗 → PaymentFailed（同一TX）
  WHEN  cardValid    → EVT PaymentCompleted → POL IssueTicketOnPaymentSuccess
  WHEN  cardInvalid  → EVT PaymentFailed    → POL ExpireApplicationOnPaymentFailure

SCENARIO システムがチケットを発行する
  ACTOR System
  CMD   IssueTicket
  EVT   TicketIssued
  AGG   Ticket
  # 対応する Payment が COMPLETED
  RULE  payment must have been completed
  # 発行時の priceSnapshot と memberAtPurchase を凍結保持
  RULE  ticket must capture priceSnapshot and memberAtPurchase
  POL   ConfirmApplicationOnTicketIssued

SCENARIO システムが申込を確定する
  ACTOR System
  CMD   ConfirmApplication
  EVT   ApplicationConfirmed
  AGG   Application
  # APPLIED または PROMOTED 状態のみ確定可
  RULE  application must be in APPLIED or PROMOTED status
  # 対応する Ticket が ISSUED 状態で存在する
  RULE  matching ISSUED Ticket must exist
  ERR   invalidStatus → InvalidStatusTransitionError

SCENARIO システムが返金を実行する
  ACTOR System
  CMD   ExecuteRefund
  AGG   Refund
  # 元 Ticket が存在し、まだ返金されていない
  RULE  original Ticket must exist and not already refunded
  # 返金額は priceSnapshot と一致
  RULE  refund amount must equal Ticket.priceSnapshot
  ERR   alreadyRefunded → RefundAlreadyProcessedError
  ERR   ticketNotFound → TicketNotFoundError
  # reason 別に異なる EVT を発行し、後続 POLICY を分岐
  WHEN  reason = MEMBER_CANCEL → EVT MemberRefundCompleted → POL CompleteCancellationOnMemberRefund
  WHEN  reason = EVENT_CANCEL  → EVT EventRefundCompleted  → POL CompleteEventCancellationOnAllRefunded

SCENARIO システムが期限切れの申込を取消す
  ACTOR System
  CMD   ExpireApplication
  EVT   ApplicationExpired
  AGG   Application
  # APPLIED または PROMOTED 状態
  RULE  application must be in APPLIED or PROMOTED status
  # 決済期限を過ぎている
  RULE  paymentDeadline must have passed
  ERR   invalidStatus → InvalidStatusTransitionError
  POL   PromoteWaitlistOnExpiration

# 申込発生で決済 Saga を開始
POLICY ProcessPaymentForApplication
  TRIGGER  ApplicationSubmitted
  CMD      ExecutePayment

# 繰り上げ通知で決済 Saga を開始（24h 期限付き）
POLICY ProcessPaymentForPromotion
  TRIGGER  WaitlistPromoted
  CMD      ExecutePayment

# 決済成功でチケット発行
POLICY IssueTicketOnPaymentSuccess
  TRIGGER  PaymentCompleted
  CMD      IssueTicket

# 決済失敗で申込を即時失効
POLICY ExpireApplicationOnPaymentFailure
  TRIGGER  PaymentFailed
  CMD      ExpireApplication

# チケット発行で申込を確定状態に
POLICY ConfirmApplicationOnTicketIssued
  TRIGGER  TicketIssued
  CMD      ConfirmApplication

# 個別キャンセル経由の返金完了でキャンセル状態を確定
POLICY CompleteCancellationOnMemberRefund
  TRIGGER  MemberRefundCompleted
  CMD      CompleteApplicationCancellation

# スケジューラが決済期限経過を検知して申込を失効させる
# [?] PaymentDeadlinePassed の発行元（cron / Temporal / 単体スケジューラ）は実装フェーズで決定
POLICY ExpireApplicationOnPaymentDeadline
...
SCENARIO 参加者が会場で受付する
  ...
  ERR   invalidTicket → InvalidTicketError
  ERR   wrongType → ParticipationTypeMismatchError
  ERR   wrongTime → NotTodayError
  ERR   alreadyCheckedIn → AlreadyCheckedInError

SCENARIO 参加者がオンラインで参加する
  ACTOR Member
  QRY   GetStreamUrl
  CMD   JoinOnline
  EVT   JoinedOnline
