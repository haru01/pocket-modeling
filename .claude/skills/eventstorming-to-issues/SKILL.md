...
- DML の POLICY / SCENARIO ブロックも抽出し `policies` / `dml_scenarios` キーに格納
- DML パーサは `eventstorming-facilitator/scripts/eventstorming_build.py` の `parse_md()` を再利用
...
- `epics/<bc>__<AGG>.md`: **AGG Epic（self-contained）**。**ビジネス背景と制約（目的・背景・制約 + BC 共通の方針）**・Zod スキーマ・不変条件（RULE）・エラー（ERR）・状態遷移・CMD/QRY 詳細（RULE 直下の `WHY` / ERR 直下の `WHEN` を併記）・受信 POLICY・発信 EVT・受け入れ条件・モジュール構造提案を 1 ファイルに集約
