...
- **Event Flow テーブルは `scenarios[]` 確定前（フェーズ1・2）は省略**し、HTML パス案内＋「フロー図はフェーズ3以降で描画」の 1 文に代える
...
- 💡 ポイント解説：そのフェーズで**初出**した DDD／EventStorming 用語のみ（書き方・上限は §2 のルールと同じ）。なければセクションごと省略
...
| フェーズ2完了 | `session` + `narratives[]`（id/kind/title。prose は全文を流さず要約 or 冒頭抜粋） |
