...
ユーザーが送る全角 `？` は「判断に迷った・決めきれない」の合図。**同一論点で連続して届いた回数**に応じて対応を変える。狙いは、推測で埋めず（SKILL.md ① 会話プロトコル）かつセッションを止めないこと。1〜2 回目でユーザーの判断を助け、それでも決まらない 3 回目は「Claude とユーザーだけでは決めきれない論点」と認めて有識者へ委ね、記録を残して前へ進む。
...
1. **有識者相談を勧める** — 「この論点は業務の有識者（ドメインエキスパート）に確認するのが確実です」と明示する。
2. **記録** — 論点を `questions[]` に open で追加。chat の 未確認事項 `Q{n}` 番号と id を揃える。`note` 冒頭に `[有識者相談推奨]` を付ける：
   ```
   dmlctl add <session>.dml.yaml --to=questions \
     --item='{id: Q3, topic: "<迷っている論点>", why: "<なぜ確信が持てないか>", status: open, note: "[有識者相談推奨] <補足>"}'
   ```
...
この論点は Claude とユーザーだけで確定するのは難しそうです。**業務の有識者（ドメインエキスパート）に確認する**のが確実です。忘れないよう未確認事項として記録しました（Q{n}、open）。
